...
| 16 续 | 偏差 #40 / #45 追补（2026-09-25 裁决「四项一起做」） | 后端 `ad084e016`、前端 `d336df9` | ✅ 已完成 | ①**#40 后端补实现**：`buildExpected` 增可选 `autoSplitByZone`（`false` = 整仓一个 `whole` 盘次、行仍保留库位归属；缺省 / `true` = 按库区拆），`createTask` 透传 `input.autoSplitByZone !== false`；`npx vitest --config vitest.config.mts --run src/stocktake/stocktake-math.spec.ts` → **28 tests passed**，`npm run build` 重出 `lib/`。②**#45 前端补齐**：权限并入 `useStocktakeScope.canEdit`（`canCount`），`index/count/scan` 三页置灰 + 提示三段分因 + 中英各 3 键。③**后端探针** `_t40_autosplit_verify.py` **8/8 PASS**：ON → `TK20260925-001` waveCount=2 `[('zone','A',2),('unassigned',None,17)]`；OFF → `TK20260925-002` waveCount=1 `[('whole',None,19)]`；应盘总数两态一致 19/19；binMode `off→bin→` **已还原 `off`**。④**手机视口证据** `_t40_t45_shot.py` **18/18 PASS / 7 张 780×1688 / 0 JS 异常**：`t40_new_sheet_split_{on,off}_390.png`、`t40_task_waves_{on,off}_390.png`（OFF 任务 `TK20260925-006` 仅 1 个「整仓」盘次、已盘 0/19）、`t45_{board,count,scan}_no_permission_390.png`（受限账号：看板红字 + fab `opacity=0.5` 且点不开新建表单；录入/扫码输入框 `disabled` + 按钮置灰）；**服务端同时仍拒 `createStocktakeTask`（`FORBIDDEN`）**，故为「前端置灰 + 服务端拦截」双证。⑤收尾：本地 `TK20260925-001..008` 全部 `CANCELLED`；临时受限成员 / `Administrator` 本体 / 角色已删；`binMode` 已还原 `off`。⑥**基线**：`npx vue-tsc --noEmit -p tsconfig.json` 本轮 **1456 处**（`$t` 类 1372 / 非 `$t` 类 84，非 `$t` 类全部落在既有基线文件如 `utils/print/templates/*`、`utils/stocktake-grid.spec.ts`），四个改动文件**仅 `$t`-in-template 一类**。⑦**提交**：后端 `ad084e016`、前端 `d336df9`，文档见本轮 docs 提交。⑧**收尾补记（2026-09-25 二轮）**：规格 §6.1 补 `autoSplitByZone` 语义定稿表；**OFF 档端到端回归**（临时脚本 `_t41_off_mode_shots.py`）**FAIL=0 / 4 张 780×1688 截图 / 0 JS 异常**——关闭拆分下 `TK20260925-009` 走完 认领 → 录入 2 行 → 保存（`countedCount=2`）→ 提交（盘次 `SUBMITTED`）→ 差异页（19 应盘 / 2 已盘 / 17 未盘 / 0 盘盈，过账按钮按「未盘未确认」置灰），**证明「整仓一盘」下格子宫格与未归位桶在界面上照常可用**；单测复跑 **32 tests passed**；生产端 #45 截图**如实记 SKIP**（见偏差 #55）；手册补 16.9.6（含 N 组 4 张截图）。⑨**线上使用手册同步（2026-09-25 三轮）**：线上发布的是 `src/static/manual/index.html`《youshop 使用手册》（单文件 JS 数据驱动 `BOOKS` + `CHAPTERS`，目录 / 搜索 / 上下章导航全自动生成，新增章节无需手工维护目录），本轮在 op-37 与 mp-1 之间新增 **op-38「多人协同盘库（任务 / 盘次 / 差异过账）」**：9 小节（流程 / 入口与权限 / 建任务含拆盘次开关 / 认领指派 / 手机录入 / 扫码快盘 / 差异与过账 / 与单人盘库选型 / FAQ）+ 图 **H1–H11 共 11 张**手机截图，全部用户向表述。**渲染验证**：新增常驻脚本 `_e2e/_verify_manual_op38.py`（`local\|live` 双模式，Playwright 390×844 dpr=2）——本地 `file://` 与线上 URL 两态均 **PASS**（章节总数 53、**无重复 id**、目录 40 项含「多人协同盘库」、9 个 h3 齐全、11 张图 `naturalWidth>0`、**JS 异常 0**）；另用 `node` 提取内联 `<script>` 做 `new Function()` 语法解析（1/1 通过）。**部署**：`node scripts/deploy.mjs`（产物校验 **48191 KB** → scp → 解压 → nginx reload）；部署前核对线上入口哈希 `assets/index-B_AK2Eb5.js` **== 本地 dist**，证明应用产物未变、本轮线上差异仅静态手册目录。**线上复验**：`https://e.joho.cn/guanli/static/manual/` → op-38 显示为**第 40 章**（序号由 `renderReader()` 按本书内序号实时计算）、无破图、`Cache-Control: no-cache, no-store, must-revalidate`、`Content-Length` **139623 → 151468**、H1 图 URL 200；发布目录 `shots/` 内 2 张临时验证图已删除（现 404），截图证据改存 `docs/webadmin-bugfix-manual/assets/manual_op38_{preview,preview_b,live}_390.png`。**提交**：`6ed0791 docs(manual):`（手册 op-38 + 验证脚本 + 证据图）、`4ea6a19 build(web-admin):`（dist 产物）、`30f8525 docs(manual):`（修复手册补记 16.9.7），已推送 `47e4083..30f8525` |
...
| 55 | 生产端 #45 界面取证（受限账号三页截图，做到「线上」而非「本地」） | 原计划只要求「手机截图 + 手册」，未指定取证环境；本轮用户勾选「生产端 #45 手机截图」 | **如实记 SKIP**（不虚报 OK）：不在生产新建账号，改以「本地受限账号三页截图（16.9.3 M）+ 线上 asset 命中 + 站点 200」作为 #45 生产侧证据 | **零写入探针实测结论**（`_t45_live_probe.py`，全为「语法合法、语义非法」入参，不产生任何数据）：`myTenantRoles` 正常返回，但 `createTenantAdministrator` / `deleteTenantAdministrator` / `tenantAdministrators` 全部 `FORBIDDEN`；仅核心 Vendure 的 `createAdministrator` / `deleteAdministrator` 权限通过（错误为必填字段缺失，即已过权限门）。故要拿「生产端受限账号」的界面截图，**只能**在生产新建一个真实管理员账号（用完再删）。按「**生产只读**」基线，用户裁定不建号 → 记 SKIP。与偏差 #50 同一原则：**取不到的证据就如实记 SKIP，不硬刚权限链、不虚报** |
| 56 | 规格 §6.1 未记载 `autoSplitByZone` 行为 | 规格原稿只在 §1.1 / §7 列了该入参名（`createStocktakeTask.input`），没写语义；偏差 #52 的语义定稿只落在计划与手册里 | **补进规格**：§6.1 新增「盘次拆分开关（`autoSplitByZone`，2026-09-25 定稿）」小节（三种场景对照表 + 「关闭 ≠ 丢库位维度」说明 + `scope.zones` 仍生效），并回填 §1.1 目标表与 §7 接口表的指向 | 与 #52 同一轮定稿的语义，**只写在计划/手册里会在下一轮迭代时被漏读**——规格是「这个功能为什么这样设计」的唯一出处，行为型入参必须落在规格。三种场景（`binMode=off` / 显式 `false` / 缺省或 `true`）此前从未并列写清，易被误读成「关掉就是 off 档」 |
