...
- **证据**：`docs/verify/gap4-batch4-*.png` 共 15 张（390×844 / dpr=2 = 780×1688，逐张人工看图核对为真实页面）；各段 console **0 error / 0 pageerror**。另有 Task 4.9 **线上手册**取证 3 张（`gap4-ops-manual-{01-toc-op40,02-chapter-top,03-selfcheck}.png`，同手机视口，见 D37）。
- **缺陷与经验沉淀**：① D28 双渠道拆分（脚本缺陷）；② D29/D30 留痕的**检索口径**与**前置条件**（是预留单编号不是订单号；渠道须有可用仓）；③ D31 是本批**最有价值的部署级发现**——入库 `dist/` 与 src 漂移会让生产静默跑旧配置，规则已沉淀；④ D35 破除「schema 不匹配 = 迁移没跑」的误判（真实机制是 `synchronize: true` 的 Auto-Sync）；⑤ D36 的教训是「生产造 fixture 前先核实渠道码」，并留档一个未处理边界（渠道删/改名后存量预留单永不释放）。
- **生产部署实况**：后端仓 commit `41cdd1f8b`（`dist/` 重建 + 新增 `prod-start-worker.js`）→ 服务器 `git pull --ff-only`（14 commits / 67 files）→ `pm2 restart vendure` → **新增并 `pm2 save` `vendure-worker`**（id 4 / fork / online，注册 12 个定时任务并每分钟执行 `release-expired-reservations` → `completed successfully`）→ 生产端到端冒烟 `PENDING_ALLOC → RELEASED`（`RELEASED_OK`，测试行已清理）。前端 web-admin 走 `node scripts/deploy.mjs`（脚本内建 `build:h5`）。
- **测试数据副作用（仅本地库 + 生产已清理）**：本地 fixture 由 `_tmp_seed_resv.mjs` 直插（用完即删）；生产冒烟插入的 `orderId=999902` 预留单及其 `RES-*` 流水在验证后**已删除**，生产库无残留测试数据；未改 vendure 任何业务文件（后端改动仅 `dist/` 重建与新增 worker 入口脚本）。
- **本批未做（属范围外，已在 D25 / D36 留档）**：共享组件 `TrendChart` 的既有渲染缺陷（本轮不改，故差异趋势改为页面内轻量条状）；「渠道被删/改名后其存量预留单永不释放」的边界处置。
