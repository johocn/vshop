...
| D28 | 批 4（Task 4.8 验收脚本缺陷） | 验收脚本原设计把 **A 段（批次状态机）与 B 段（预留单 / 看板）统一到单一渠道**，导致 A 段必然 `ENV-FAIL: 既无 PRINTED 批次、也无候选订单可造批次` | 本地库中**批次存量与可配货订单都在 `__default_channel__`**，`shop-a` 无订单 | 已改为 **A/B 双渠道**：新增 `WA_B4_CHANNEL`（默认 `__default_channel__`）与 `WA_B4_RESV_CHANNEL`（默认 `shop-a`），`admin_login(channel)` / `inject_channel(pg, code)` 均参数化，脚本按段注入渠道并**分段登录**（A 段批次状态机、B 段预留单/看板）。另修一处定位器缺陷：`grid.locator('.g')` 为空（`grid` 本身即 `.grid .g`）→ 改 `grid.last.click()`，否则 30s 超时 |
| D29 | 批 4（留痕检索口径，D5 补充） | 释放留痕 `bizCode = RES-<预留单id>`，**不是订单号** | 释放的语义单位是「预留单」（一个订单可有多张预留单） | 已在手册 20.5① / 20.7 与使用手册 `op-40` 第 5 节显式标注「是预留单编号，不是订单号，按订单号检索会查不到」 |
| D30 | 批 4（留痕前置条件，Task 4.2 实测口径） | 释放留痕要求该渠道**有可用仓**（默认物理仓，退而取虚拟仓）；两者皆缺时释放**仍成功但跳过留痕**（仅 `warn`） | `recordReleaseLedger` 需要 `stockLocationId`，查不到仓便无处挂账 | 已按此实现；本地 `shop-a` 先调 `ensureTenantInventoryLocations` 补建虚拟仓后复现出 `RES-7` 流水；**生产 `t1` 渠道无仓 → 实测命中 warn 跳过分支**（功能正常，仅少一条留痕）。手册 20.5② / 20.8⑤ 留档 |
| D31 | 批 4（Task 4.9 · 部署口径，**致命**） | 入库的 `packages/dev-server/dist/dev-config.js` 仍是 `runTasksInWorkerOnly: false`，与 `dev-config.ts` 的 `true` **不一致** | 与 D12 同源：后端 `dist/` 入库，但历史提交只改 src 未重建 dist；而 `prod-start.js` 加载的正是 dist | 已用 `npx tsc -p packages/dev-server/tsconfig.prod.json` 重建 dist（仅 `dist/dev-config.js` 变化）并随 `41cdd1f8b` 提交。**规则：凡改动被 `tsconfig.prod.json` 覆盖的文件（`dev-config.ts` 等），必须重建 dist 并一并提交**，否则服务器 pull 后 D1 结论不成立 |
| D32 | 批 4（Task 4.9 · pm2 名称与命令） | 计划写 `pm2 restart youshop-api youshop-worker`，服务器**没有** `youshop-api` | 服务器 pm2 真实条目为 `nshop`(cluster) / `strapi`(fork) / **`vendure`**(fork, script=`prod-start.js`, cwd=`packages/dev-server`, interpreter=`node`) | 按 D13 实读后照抄：server 沿用既有 `vendure`（`pm2 restart vendure --update-env`），worker 新增 `vendure-worker`（同 cwd/interpreter）并 `pm2 save`；手册 20.8① 固化「动 pm2 前先 `pm2 list` / `pm2 describe` 实读」。计划原文的 `youshop-*` 命名作废 |
| D33 | 批 4（Task 4.9 · 缺生产 worker 入口） | 仓库内**不存在**生产 worker 入口 | 原只有 `dev:worker`（`node -r ts-node/register … index-worker.ts`），生产不能用 ts-node | 新增 `packages/dev-server/prod-start-worker.js`（dotenv + `NODE_ENV=production` + DB 兜底 + `require('./dist/index-worker')`，注释说明「worker-only 故必须常驻」），随 `41cdd1f8b` 提交 |
| D34 | 批 4（Task 4.9 · TTL 口径更正） | 计划第 5 条写「改完需重启 worker（自定义字段值在 ctx 构建时读取，无需重启 server）」 | **实测口径不同**：`ttlMinutes(ctx)` 仅在**下单预占**时被调用、当场算出 `expiresAt` 落库；worker 释放时只比较已落库的 `expiresAt`，不再读渠道配置 | 手册 20.8④ 与使用手册 `op-40` 第 4 节更正为「**改 TTL 无需重启 worker**——只影响此后新建单，存量单沿用原 `expiresAt`」；仅改**代码**才需 `pm2 restart vendure-worker` |
| D35 | 批 4（Task 4.9 · 启动日志与迁移） | 生产 server 每次启动都打印 `Your database schema does not match…` + 一串 `ALTER TABLE … ADD …`，且 `migrations` 表为**空**，极易被误判为「迁移没跑、生产 schema 落后」 | `dev-config` 的 `getDbConfig()` 对 postgres 返回 `synchronize: true`，且**在 `synchronize: false` 之后展开** → 实际生效 `true`：提示先打印，随后由 Auto-Sync 当场建列，故 `migrations` 表本就为空 | 已直连库核对 8 项新增列**均已存在**（`stock_reservation.expiresAt`、`pick_batch` 的 `handoverAt/handoverTo/reviewedAt/exceptionAt/exceptionNote`、`channel.customFieldsReservationttlminutes`、`collection.customFieldsIcon`），确认无需补跑迁移；手册 20.8「schema 说明」留档以免后续再误判。另：error 日志中 `Cannot find module './stock-city-filter'` 为**历史条目**（当前 08:20 启动 0 条 error，且该 `lib/` 文件在服务器存在） |
| D36 | 批 4（Task 4.9 · 生产端到端 fixture 渠道） | 首次生产冒烟用 `shop-a` 造过期预留单 → **未被释放**，一度误判为缺陷 | 生产 `channel` 表**没有** `shop-a`（真实码为 `__default_channel__` / `t1` / `t24` / `official-01…20` / `test-marketplace-shop`）。任务按 `tenantChannelId` 反查渠道失败 → **退回默认渠道 ctx** → 该行过滤不中 → `released=0` | 改用真实存在的 `t1` 重测：`PENDING_ALLOC → RELEASED`（`RELEASED_OK`），日志 `预留单超时释放 1 单`，测试行随后清理（含 `orderId=999902` 的中间试验全部清除）。**教训：生产造 fixture 前先 `select code from channel` 核实渠道码**。同时暴露一个**未处理边界**：渠道被删/改名后，其存量预留单的 `tenantChannelId` 反查不到渠道 → 退回默认渠道 → **永不释放**（本批不改，留档待评估） |
| D37 | 批 4（Task 4.9 · 线上手册取证，计划外补充） | 计划 Task 4.9 只要求「站点可访问」，未要求对**线上使用手册**做截图取证 | 交付硬规则要求「每次功能测试必须手机视口截图」，而本任务的交付含线上手册 `op-40`，故补一个**只读**脚本取证 | 新增 `_e2e/_verify_gap4_ops_manual.py`（390×844 @dpr2 / is_mobile / has_touch；**不登录、不发写请求**，故不套用批 1/3/4 的 `FORBIDDEN_HOSTS` 写护栏）：首页 → 进「租户运营手册」分册 → 开目录 → 断言 `.toc-item[data-id="op-40"]` → 点进该章 → 逐条断言 5 条必含内容（`vendure-worker` / `Registered scheduled task` / `Executing scheduled task` / `pm2 list` / `reservationTtlMinutes`）+「默认 30 分钟」+「不用重启」→ 三张图（目录条目 / 章节首屏 / 四步自检小节）落 `docs/verify/gap4-ops-manual-*.png`，**PASS（失败 0）**，0 pageerror / 0 console.error。**踩坑留档**：该手册默认停在**首页**、目录只渲染**当前分册**的章节，`state.book` 为 null 时目录为空 → 首版脚本直接开 `#btnMenu` 得到 `count=0`，须先点 `.book-card` 进分册 |
| D38 | 批 4 收尾（生产数据边界修复，计划外） | 计划未覆盖「渠道被删除/改名后其存量预留单永不释放」（上轮仅以 D36 留档待评估，未修） | 用户指示「先处理生产数据边界问题」 | 后端 `df8f43ed4`：① `releaseExpired` 新增 `options.tenantChannelId`，**释放范围与 ctx 解耦**（不传仍取 ctx 渠道，既有口径不变；传 `null` 只收窄 `IS NULL`）；② worker 任务按分组返回的 code 逐组传入，反查落空时告警并把 `orphanChannels` 计入返回；③ `recordReleaseLedger` 加守卫：预留单自身 `tenantChannelId` ≠ ctx 渠道 → **跳过留痕并告警**（否则把别的渠道的释放记到默认渠道账上、污染对账）。单测 11 → **13 例全绿**；`lib` 产物重建入库（D12）；生产实测孤儿渠道单 `#20` 一个 cron 周期内 `PENDING_ALLOC → RELEASED`，日志四连（task warn 渠道已不存在 / service warn 跳过流水留痕 / info 释放 1 单 / info 涉及 1 个渠道其中 1 个已不存在），测试行已清理（`remain 0`）｜**取证口径**：本项**无 UI 面可截图**——孤儿渠道的行在其归属渠道已不存在的前提下，任何渠道的「预留单」页都看不到它，故以**库状态 + worker 日志**为证据（同 20.8⑤ `RELEASED_OK` 口径）；**前端侧**同步补文档并重跑门禁：线上手册 op-40「常见问题」增一条 FAQ（渠道被删除/改名后仍会释放、只是无法写流水），`_e2e/_verify_gap4_ops_manual.py` 增第 5 步断言（展开该 FAQ 核对「渠道编号」/「库存流水」/「已不存在」）并补第 4 张同视口取证图 `gap4-ops-manual-04-faq-orphan-channel.png`，重跑 **PASS（失败 0）**；修复手册 20.8⑧ 同步 |
| D39 | 批 4 收尾（遗留待办清理，计划外） | 三处遗留：① 本地 2 个测试用后台进程（`npm run dev:server` PID 3124 / `npm run dev:worker` PID 25620）仍在跑；② 服务器 `vendure-error.log` 有 1432 行历史噪音；③ 服务器 `/tmp` 有 2026-09-19 遗留的 `_tmp_locs_clean2.js` | 用户指示「先不管 D25 组件，继续处理其他待办」（除 D25 `TrendChart` 外全部收口） | ① 经 `Win32_Process` 读出命令行确认为本任务测试所启（`job-da12b14a` / `job-de449d65`）后 `taskkill /T /F` 整树终止，复查两 PID 已不存在；② 先只读诊断再动手：噪音为 `Cannot find module './stock-city-filter'`（21 条）+ `'@vendure/shop-template-plugin'`（29 条），**决定性验证** = 在服务器复现日志里那条 require 链 `node -e "require('…/cjk-plugin/lib/index.js')"` → **`REQUIRE_CHAIN_OK`**，且 lib 产物存在并被 git 跟踪、`@vendure/shop-template-plugin` 软链 `RESOLVE_OK` → 判定为历史 boot 残留、不会再报，随后**只截断 error 日志**（1432 → 0 行，保留 out 历史）；③ 与 `_tmp_logdiag.sh` / `_tmp_finalize.sh` 一并删除，`/tmp` 已无 `_tmp_*`。修复手册 20.8「遗留噪音」callout 同步改写为「已核实并清理」并列出两类条目与验证命令 |
| D40 | 批 4 收尾（D25 追单修复，**推翻 D25 的「本轮不改」结论**） | 用户指示继续处理 web-admin 开发任务，D25（共享组件 `TrendChart` 渲染缺陷）是**唯一遗留项**，故本轮直接修组件本身 | D25 原本以「不改共享组件、差异趋势改用页面内条状」绕过（作业分析页那条路径**保持不变**，无回归）；但经营数据「近 7 日销售趋势」的缺陷仍在，属真实用户可见问题 | **根因**：H5 下 `<canvas>` 被编译为 `<uni-canvas>` + 内层 `canvas.uni-canvas-canvas`，内层画布由 uni **hidpi 包装器**接管（缓冲 = 元素尺寸 × dpr、上下文打 `__hidpi__` 标记、原型方法把 CSS px 坐标**自动**乘 dpr）；组件原实现**又** `ctx.setTransform(dpr,0,0,dpr,0,0)` → 坐标乘两次（dpr²），整幅图右下偏移被裁切。**次因**：尺寸原取 `getBoundingClientRect()`（亚像素），与 uni 取整后的缓冲尺寸差 1px 反复互踩。**修复**（`src/components/TrendChart.vue`，单文件、全仓仅 `pages/data/dashboard/index.vue` 一处引用）：删 `setTransform`、全程按 CSS px 作图、尺寸改 `offsetWidth/offsetHeight × dpr` 取整并加 `width/height` 守卫。**验收**：新增 `_e2e/_verify_d25_trend_chart.py`（`document-start` 注入探针拦截全部 2D 调用，把参数按调用时变换还原成 CSS px 后断言 A 尺寸 / B **无二次缩放护栏**（`setTransform` 的 a/d 必须 = 1）/ C 不越界 / D 空态居中 / E 有数据态齐全；视口 390×844 @dpr2 `is_mobile` `has_touch`；只拦 `salesTrend` 一条、其余走真实后端）。**缺陷态**（`git stash push -- web-admin/src/components/TrendChart.vue` 还原后重跑同一脚本）**5 失败 / 退出码 1**（`setTransform(a,d)=[(2,2)]`、横坐标越界至 `571.6 > 332`、空态中心 `(303.8,187.2)` vs `(166.0,93.5)`）→ 取证 `gap4-d25-before-{empty,data}.png`；**修复态 0 失败 / 退出码 0** → `gap4-d25-{empty,data}.png`（共 4 张，均 390×844 @dpr2）。修复手册新增 **20.9** 节并列两态取证，`verify:manual` PASS。**线上复验**（用户追加要求；生产 `e.joho.cn/guanli` 只读，`WA_SHOT_BASE`/`WA_API_ADMIN`/`WA_D25_CHANNEL=t2`/`WA_D25_TAG=online-`）：D25 断言 **PASS 12/12**（`setTransform=[]`、空态中心 `(166.0, 93.5)` 与期望完全相等、7 日期标签、`¥318/0/9`、`arcs=14`），新增 2 张线上取证图 `gap4-d25-online-{empty,data}.png`；线上唯一 FAIL 为 `inventoryHealth` 未授权（**与 D25 无关**），经 `WA_D25_NO_INJECT=1` 对照（不注入渠道反而扩到 `dashboardOverview`/`inventoryHealth`/`categoryTop` 三条 FORBIDDEN）判定为**账号角色权限**问题，记为新待办 **D41**（不改本次交付） |
| D41 | 线上复验中发现的**独立待办 → 已修（计划外）** | 线上复验 D25 时，`data` 态的「0 console.error」断言命中 1 条：`fetchInventoryHealth failed … You are not currently authorized to perform this action` | 该账号（`guoxinnanshan@163.com`；`myTenantAccess` 可见渠道含 `t2`）对 `inventoryHealth` 无权限，界面表现为「经营数据」看板最下方「库存健康」卡的「缺货 / 总SKU」显示为「–」。**已排除是渠道注入造成**：加 `WA_D25_NO_INJECT=1`（不注入渠道、沿用登录后默认渠道）复跑，FORBIDDEN **反而从 1 条扩到 3 条**（`dashboardOverview` / `inventoryHealth` / `categoryTop` 全部 `FORBIDDEN`）→ 属**账号角色权限**问题，与 `TrendChart` 渲染完全无关 | **核实（实证，全部只读）**：① 用生产租户账号 `guoxinnanshan@163.com`（id=15）读 `me{channels{permissions}}` → 该账号在渠道 t2 的权限集**含 `ViewDashboard` / `ReadCatalog`，不含 `ViewStock`**；② 逐个探测（`vendure-token` 必须传渠道 **token** `66ruvnhh34svhckaa2i`，填 code `t2` 会被判为无效渠道）：`dashboardOverview` / `salesTrend` / `categoryTop` **三查全 OK**、`stockLocations` OK（`core` 的 `@Allow(ReadCatalog, ReadStockLocation)`）、**唯一 403 = `stockLevels`**（`inventory-plugin` 的 `@Allow(ViewStock)`）；③ 对照权限定义：`ViewStock` 是 inventory-plugin 的**超管语义全局库存权限**，**不在租户白名单** `cjk-plugin/src/tenant/tenant-member.service.ts` 的 `inventory` 组（该组只有网点增/改/删），且 `assertBusinessPermissions` 会拒绝白名单外权限 → **角色管理页根本无从勾选**。**定性**：既**不是**「租户角色授权缺失」（不加 `ViewStock`——它是入库/出库/调拨/盘点等全局库存单据的同一把钥匙，授予租户会连带打开超管语义接口），也**不是**「前端应隐藏卡片」（数据拿得到），而是**前端取数走错了路径**；此前「`NO_INJECT` 下 FORBIDDEN 扩到 3 条」只是落到**默认渠道**（该账号在默认渠道零权限）的假象。**修复**（前端 `src/apis/inventory.ts`）：`fetchInventoryHealth` 改走 cjk-plugin **租户级** `inventoryStockPage`（`@Allow` 含 `ReadCatalog`，实测 OK；`totalSku = summary.skuCount`、`outOfStock = summary.outCount`，与「库存明细页」**同源同口径**，不再前端二次推导），并删除随之成为死代码的核心 `fetchStock` / `StockRow`（全仓仅此处引用）。**验收**：新增 `_e2e/_verify_d41_dashboard_inventory.py`（租户账号登录 + 注入渠道 token；断言 A 控制台/页面零错误且无 `authoriz` / B 三格均为数字 / C **UI 与 API 逐字一致** / D 卡片滚动入视口截图；退出码 0/1/2），以**部署边界**作为两态切换点：**缺陷态 2 失败**（`['93','−','−']` + 1 条 `not authorized`）→ 取证 `gap4-d41-before-health.png`；`npm run build:h5` EXIT=0 → `node scripts/deploy.mjs` 部署 → **修复态 0 失败**（`['93','0','10']`，C 三项 `93/0/10` 全等）→ `gap4-d41-after-health.png`。**连带消除 20.9 线上唯一 FAIL**：D25 线上探针（同账号同渠道）由「1 失败」变为「**0 失败（PASS 12/12）**」，两张线上图按修复后产物重录。修复手册新增 **20.10** 节并列两态取证。**同类边界（本次未修、留档待评估）**：同页 `adjustStock`（`setVariantStock`）亦 `@Allow(ViewStock)`，租户账号在「调整库存」上同样会 403（仅静态读数，未做写请求实测）；如需支持应改走 cjk-plugin 租户级 mutation |
| D42 | D41 的**同类边界 → 已修（计划外）** | D41 留档的边界：同页 `adjustStock`（`setVariantStock`）亦 `@Allow(ViewStock)`，租户账号「调整库存」同样会 403（当时仅静态读数） | 用户指示「先评估 adjustStock 的修复方案」→「按方案 A 落地，先执行 t2 环境测试」 | **方案评估（选 A）**：A = 改走 `createStockDoc(type:'STOCKTAKE')`（推荐）；B = `myShopStockAdjust`（**排除**：`ManageOwnShop` + 服务层 `requireMyShop()`，语义是「店主/自有 Shop」，渠道租户无 `Shop`）；C = 后端放宽 `setVariantStock`（**排除**：需另一条后端部署链且无审计留痕）。**t2 写能力实测（先证可写，再改前端）**：临时探针 `_e2e/_tmp_d42_write_probe.py`（用后已删）在渠道 t2 对 1 个 SKU 裸写 → `createStockDoc(STOCKTAKE)` 可通、**虚拟仓写入未被拒**（t2 为纯虚拟模式 `physicalStockEnabled=false`，兜底仓 = `t2 虚拟仓` id=6）、`onHand 26 → 27 → 26` **回改复原成功**、单据在「单据中心」可见；提前发现契约细节 **`StockDocItemInput.qty` 是 schema 必填（`Int!`），`realQty` 只是覆盖值**（只传 `realQty` 报 `Field "qty" of required type "Int!" was not provided`）→ 两个都传同值。**线上实测缺陷态**：走真实 UI 路径点「调整」→ 目标 27，返回 `FORBIDDEN / path:["setVariantStock"]`，**数字未变**（onHand 仍 26），且 Toast 弹出整段 GraphQL 报错原文。**修复（前端 3 处 + i18n 2 处，无后端改动）**：① `src/pages/inventory/stock/index.vue` 的 `onConfirmAdjust` 改 `createStockDoc({type:'STOCKTAKE', remark, items:[{variantId, toStockLocationId, qty, realQty}]})`，成功 Toast 带**单据号**（`已调整 {code}`），弹层与函数注释同步改写；② `src/apis/inventory.ts` **删除死代码 `adjustStock`**，文件头「校准结果」把 `setVariantStock` 标为**已弃用（D42）**；③ i18n 两语言包 `adjust.done` 带 `{code}`、新增 `adjust.remark`（`手工调整（库存明细页）`，用于与真盘库区分）。**验收**：新增 `_e2e/_verify_d42_stock_adjust.py`（租户账号登录 + 注入渠道 token + 真实 UI 路径「搜索 SKU → 点调整 → 填目标 → 点确定」；断言 A 控制台无 `authoriz` / A-2 Toast 非失败态 / B 基线留档 / C **改数生效**（API 回读 onHand == 目标，不信 UI）/ D **留痕落库**（STOCKTAKE 单据 +1 且新单 code 出现在 Toast）/ E **强制复原**（仅 C 通过时用单据写回原值）/ F 手机视口截图；退出码 0/1/2），以**部署边界**为两态切换点：**缺陷态 `1 / 4 失败`**（Toast 为 `FORBIDDEN` 原文、onHand 未变 26、单据 `2 → 2`）→ `gap4-d42-before-{sheet,row}.png`；`npm run build:h5` EXIT=0 → `node scripts/deploy.mjs`（产物 48738 KB）→ **修复态 `0 / 0 失败`**（Toast `已调整 ST-MUIEVULQ-0B0`、onHand 27、单据 `2 → 3` op=15、复原 `ST-MUIEVZWQ-0GQ → 26`）→ `gap4-d42-after-{sheet,row}.png`（共 4 张 390×844 @dpr2，UI 上「库存总量」`36 → 37`、货值 `¥3600 → ¥3700` 自证）。修复手册新增 **20.11** 节并列两态取证。**边界留档**：手工改数现在会进「单据中心」的 STOCKTAKE 列表，后续若按单据类型做盘库统计需按 `remark` 区分「真盘库 / 手工改数」；验收脚本含生产写操作，默认锁定测试 SKU `P1789043229213` 并强制复原（`WA_D42_SKU` 可覆盖） |
| D43 | D42 的**连带影响 → 已修（计划外）** | 用户指令「请评估手工改数进单据中心对盘点统计口径的影响」——D42 让手改单复用 `stock_doc(type='STOCKTAKE')`，与「盘点任务过账单」同 type，需核对各统计口径 | 用户指示评估 → 选择「A + C：作业员明细排除 STOCKTAKE + remark 稳定化（推荐）」 | **影响面核对（判据 = 该统计到底读哪张表；全仓检索 `StockDocEntity` 只有 2 个读者）**：真正会被误读**只有看板「作业员明细」**（`pages/data/dashboard/index.vue` 取 `stockDocList({pageSize:100})` → `groupByOperator` **不过滤 type**，手改 1 个 SKU 即该操作人单据数 +1、改 26 件即件数 +26，且手改密集时挤占「最近 100 条」窗口 → 系统性低估真实作业量）；**单据中心「盘库」页签**属混列（靠备注可辨）；**不受影响 5 项**——盘点差异趋势条 + 差异率（`stocktake_task` + `stocktakeDiff(taskId)`）、盘点任务数 KPI（`countStocktakeTasks` 读 task）、任务内作业量（`stocktakeStats` 读 `stocktake_wave/line`）、库存行成本价 / 货值（`loadLatestCost` 显式白名单 `type IN ('PURCHASE','TRANSFER')`，故 D42 截图 ¥3600→¥3700 只是 +1 件 × 成本 100）、**预期内 1 项**——ledger 流水记 `bizType='stocktake'` + `bizCode`=手改单号（可追溯，正是 D42 目的）。**根因细节**：真盘库过账单由 `stocktake.service.ts` 过账生成（`remark='盘点任务 {code}'`，被 `stocktake_task.postedStockDocId` 反查），手改单同 type 且无任务引用 → 可靠判据只有「有无 task 引用」或「备注前缀」；原备注整条走 i18n（`inventoryStock.adjust.remark`），**切英文站即变样、作为判据是脆的**。**修复（纯前端 4 处 + 单测，无后端改动）**：① `src/utils/ops-report.ts` 新增口径纯函数 `opsCountableDocs()`（排除 `STOCKTAKE`，注释写明「真盘库作业量已由 `stocktakeStats` 盘次/应盘行口径覆盖，手工改数是数据修正不是作业量」）；② `pages/data/dashboard/index.vue` 改为 `groupByOperator(opsCountableDocs(docs.items), w)`；③ `src/apis/stock-doc.ts` 新增导出常量 `MANUAL_ADJUST_REMARK_FLAG = 'MANUAL-ADJUST'`；④ `stock/index.vue` 备注改为 `` `${FLAG} | ${本地化文案}` ``（前缀作判据、后缀保可读）；⑤ `ops-report.spec.ts` 增单测（`node --test` **14/14 PASS**）。**验收**：新增 `_e2e/_verify_d43_ops_counter_scope.py`（租户账号 + 渠道 token + **真实 UI「作业分析」视图**读作业员明细卡合计；**正反对照造单** = 1 张 `from == to` 同仓 **净零 TRANSFER**（护栏：证明聚合本身没坏）+ 2 张 STOCKTAKE 手改单（反对照），要求增量**恰好 1 单 / 1 件**；另断言库存收尾回基线 / 0 报错 / 手机视口截图）。**踩坑**：首轮基线被读成 0（手改 3 张单后增量算成 23）——作业员明细卡先以空态渲染，读表前必须 `expect_response` 等 `stockDocList` 请求真的返回，不能只等卡片可见。两态：**缺陷态 `1 / 2 失败`**，增量 **3 单 / 54 件**（27 + 26 + 1，两张手改单的 `qty` 被当成「件数」）→ `gap4-d43-before-ops-counter.png`；`npm run build:h5` EXIT=0 → `node scripts/deploy.mjs`（产物 48738 KB）→ **修复态 `0 / 0 失败`**，增量 **1 单 / 1 件** → `gap4-d43-after-ops-counter.png`；同账套作业员明细合计由 `26 单 / 240 件` 收敛为 `18 单 / 28 件`（剔除全部 STOCKTAKE 单据的直观证据）。修复手册新增 **20.12** 节并回改 20.11 ⑥。**边界留档**：单据中心「盘库」页签仍混列（区分看备注：`MANUAL-ADJUST | …` = 手改 / `盘点任务 {code}` = 真盘库）；若要精确区分并各自成组，最稳是后端 `stockDocList` 按 `postedStockDocId` 反查回传任务号；「最近 100 条」窗口上限为既有缺陷、未扩大范围；验收脚本每轮产生 3 张测试单（带「验收/可忽略」备注、移库净零、库存强制回基线） |
| D44 | D43 留档的**边界 → 已修（计划外）** | D43 留档：「单据中心『盘库』页签仍混列（真盘库过账单 / 手工调数单同 type），靠备注区分是脆判据；若要精确区分，最稳是后端 `stockDocList` 按 `postedStockDocId` 反查回传任务号」 | 用户指令「请评估后端按 postedStockDocId 反查回传任务号的方案」→ 评估后选择「**落 B（含索引，推荐）**」：后端加 `taskId/taskCode` + 补 `(tenantChannelId, postedStockDocId)` 索引 + 前端单据中心区分「盘库过账 / 手工调数」 | **方案评估**：A = 纯前端按 `remark` 前缀判定（**排除**：备注可被后续版本改写、且 i18n 化后语义漂移，脆）；B = **后端反查**（选中）；C = 新增独立 `docType`（**排除**：改动历史数据口径、影响面大）。**后端（`cjk-plugin`，commit `1a34ce866`）**：① `stocktake-task.entity.ts` 加 `@Index(['tenantChannelId','postedStockDocId'])`；② 新增幂等 migration `migrations/migrate-stocktake-posted-doc-index.ts`（`CREATE INDEX IF NOT EXISTS "idx_stocktake_task_posted_doc"`，走 `OnApplicationBootstrap`，失败仅 `console.error`——因生产/本地 `synchronize` 关闭、实体 `@Index` 不自动落地）；③ `inventory/stock-doc.service.ts#listDocs` 末尾：取本页 `type === 'STOCKTAKE'` 的 doc id 列表（**`In([])` 必须短路**，否则 TypeORM 生成 `IN ()` 报错）→ 查 `StocktakeTask({ tenantChannelId: String(ctx.channelId), postedStockDocId: In(ids) })` → 回填 `taskId/taskCode`；④ `plugin.ts` admin SDL `StockDocSummaryRow` 增 `taskId: ID` / `taskCode: String`。**关键口径发现（租户键不对称）**：`stock_doc.tenantChannelId` 存的是**渠道 code**（`t2`），而 `stocktake_task.tenantChannelId` 存的是 `String(ctx.channelId)`（`37`）→ **两者不可 join 租户键**，只能「按本页盘库单 id 反查 + 渠道二次过滤」，索引也据此建在两列上。**前端（`web-admin`，4 文件）**：`src/apis/stock-doc.ts` 类型 + 选择集加 `taskId/taskCode`；`pages/inventory/stock-doc/index.vue` 卡片增来源行（`taskCode` 有值 → 「盘库过账 · 任务号：TK…」；为空 → 「手工调数」）；`locale/zh-Hans.json` + `en.json` 同步 `fromTask/fromManual/taskCodeLabel`。**验收**：新增两态脚本 `_e2e/_verify_d44_stockdoc_task_source.py`（租户账号 + 渠道 token；`ALLOW_WRITE` 环境变量护栏，默认只读；含 `cleanup_stale_tasks()` 取消同名遗留未终态任务；正向 fixture 走完整链路 `createStocktakeTask → stocktakeWaves → claimStocktakeWave → stocktakeExpectedLines → saveStocktakeCounts(base+1) → submitStocktakeWave → postStocktake`，再用手改单绝对写回基线，库存 `26 → 27 → 26` 净零）。**生产数据决定性发现**：`stocktake_task` **21 行全 `CANCELLED`、`postedStockDocId` 全 NULL**，即从无任何任务成功过账；且**零差异无法过账**（`post()` 对 `plan.items.length === 0` 直接抛「没有需要过账的差异」）→ 故必须真造一张过账单。两态：**缺陷态（前端部署前只读跑）S5/S5b 失败** → `gap4-d44-before-stockdoc-center.png`；`npm run build:h5` EXIT=0 → `node scripts/deploy.mjs`（48739 KB）→ **修复态 `0 / 0 失败`**：真过账单 doc **43**（`ST-MUIKWABW-0GS`，remark「盘点任务 TK20260927-001」）→ `taskId=24 / taskCode=TK20260927-001`；手改复原单 doc **44**（`ST-MUIKWAPV-0K8`）→ `taskId=null / taskCode=null`；UI 标签实读 `['手工调数','盘库过账',…]` + `['任务号：TK20260927-001']`，0 JS 异常 → `gap4-d44-after-stockdoc-center.png`（共 2 张 390×844 @dpr2）。修复手册新增 **20.13** 节并列两态取证。**边界留档**：作业员明细「最近 100 条」窗口上限为既有缺陷未扩大范围；`TK20260927-001` 为终态过账任务不可取消，长期留 t2 作正向 fixture |
| D45 | D44 验收中被卡住的**真实缺陷 → 已修（计划外，与 D44 同批收口）** | 写模式跑 D44 时 `claimStocktakeWave` 报「**当前账号不是本店人员，无法认领盘次**」——一度疑似 fixture 问题，实为**真实缺陷** | 用 AskUserQuestion 询问处理方式，用户选择「**立即修 + 一气呵成收口**」 | **根因（键错位）**：`ctx.activeUserId` / `session.user.id` 是 **`User.id`**，而 `tenant_member.administratorId` 存的是 **`Administrator.id`**；`Administrator` 通过 **`user` 关系**（`where: { user: { id } }`）关联 User，**不是 `userId` 属性**（写成 `where: { userId }` 报 TS2353，须 `where: { user: { id } } as any`）。直接拿前者比后者 → **恒不匹配**。**生产实证（t2 唯一真实店长）**：`administrator` id=**53** / `userId`=**15**（不等！）；`tenant_member` id=48 / `administratorId`=**'53'** / `channelId`=37（= t2）→ `currentOperator` 拿 `activeUserId=15` 查 `administratorId='15'` 查空 → 回 `{id:null}` → 认领被拒。**这解释了 D44 的前置异常**：21 个 `stocktake_task` 全 `CANCELLED`、`postedStockDocId` 全 NULL = **从来没有任务成功过账**（`Administrator.id ≠ User.id` 的账号整条「认领 → 录入 → 提交 → 过账」链路完全不可用）。**同源隐患（顺带核实并修）**：`auth/tenant-enabled.guard.ts` 同样按 `session.user.id` 直查 member → 恒空 → 「停用人员(`enabled=false`)」「首登强改密(`mustChangePassword`)」两道闸门被**静默绕过**（fail-open）。**判据来源**：同插件 `tenant-member.service.ts#memberToView` 已是 canonical 正确写法（先 `administratorService.findOneByUserId(ctx, activeUserId)` 换键取 `Administrator`，再比 `String(myAdmin.id) === String(member.administratorId)`）→ 本次是**向既有正确写法对齐**，非新造语义。**修复（后端 2 文件，commit `71547ec04`，7 files / +53 −28）**：① `stocktake/stocktake.service.ts#currentOperator` 先换键取 `admin.id` 再查 `TenantMember`，且**按本渠道收口**（`channelId = tenantOf(ctx)`，与 `assignWave` 校验同口径，避免跨店误取同名人员）；② `tenant-enabled.guard.ts` 同源换键，使两道闸门真正生效。`lib/` 编译产物重建入库（同 D12 规则）。**部署**：本地 build → commit+push → 服务器 `git pull --ff-only` + `pm2 restart vendure`（部署后首跑曾遇 502 Bad Gateway = 服务未就绪，`admin-api HTTP 200` 后重跑通过）。**验收（证据链非单点）**：修复后重跑 D44 脚本 `认领 → 录入 → 提交 → 过账` 全通（`TK20260927-001 → doc 43`）即证明 `currentOperator` 已正确解析本店操作人；守卫侧以**不回归**为准——同一次运行中登录 / 看板 / 库存 / 单据中心**全部 admin 请求正常**（守卫对每个 admin 请求执行），页面 `0 pageerror / 0 console.error`。**行为变化告知**：守卫修复后，**已关联 `tenant_member` 且 `enabled=false` / `mustChangePassword=true` 的账号会被真正拦下**（设计意图）；**未关联 member 的账号语义不变照常放行**；已核对 t2 member 48（`enabled=t` / `mustChangePassword=f`）确认**无账号被误锁**。修复手册新增 **20.14** 节。**遗留**：原注释称「照 `pick-batch.admin.resolver` 实现」，全仓检索确认**该 resolver 无同型错位**，故范围收敛在这 2 处；建议后续把「`User.id` → `Administrator.id` → `TenantMember`」换键封装为共享 helper |
| D46 | D43 留档的**既有缺陷 → 已修（计划外）** | D43 留档：「作业员明细的『最近 100 条』窗口上限是既有缺陷（本次未扩大范围），单据量大的店需留意」——该口径的前端实现受 `stockDocList` 的 `pageSize` 硬顶约束 | 用户指令「请评估作业员明细窗口上限的优化方案」→ 给出代码级评估 + 内联可视化三方案对比后，用户选择「**B 后端聚合（推荐）**」 | **缺陷定性（三方证据）**：① 服务端 `clampPageSize = Math.min(100, n)` 硬顶 100 → 前端调大 `pageSize` **无效**（验收脚本实测 `pageSize=1000 → totalItems 151 / 实回 100`）；② 前端原调用 `fetchStockDocList({pageSize:100})` **未传 `from/to`** → 拿到「全时段最新 100 条」再在浏览器里按窗口过滤（双重浪费）；③ 生产 t2 实测 `stock_doc` 31 条（全在 7 天窗口内），距硬顶约 3 倍余量 → 属**容量天花板型静默缺陷**（窗口内单据 >100 条时按 `createdAt DESC` 截掉的是**较老**单据 → **低频作业员整行消失、合计系统性低估**，不报错）。**方案评估（四选一）**：A 前端分页累加（**排除**：上限仅 100→1000、非快照分页有丢读/重复风险、请求数随数据增长）；**B 后端聚合（选中）**：新增 `stockDocOperatorStats(from,to)` 在 SQL 侧 `GROUP BY operator`（排除 STOCKTAKE），上限消失、1 次请求、无分页漂移；C 整页服务端聚合（含 `fetchPickBatches` / `fetchStocktakeTasks` 两处同类硬顶，记为更大范围选项）；D 调大 pageSize（**不可行**，被硬顶）。**口径等价性设计（与 D43 逐条对齐）**：`COUNT(DISTINCT d.id)` 计单据数（left join 明细后行不膨胀，**无明细单据仍计 1 单**）+ `COALESCE(SUM(i.qty),0)` 计件 + `COALESCE(d.operator,'')` 折空串（「未记录操作人」排最前，与旧前端 `localeCompare('')` 语义一致）+ 排序「单据数降序 / 操作人升序」，日期区间仍**绑定 Date 实例**（`stock_doc.createdAt` 无时区，ISO 串直传偏 8 小时——19 章已修同款口径）。**后端落地（`cjk-plugin` 3 处，commit `423ed9df0`）**：① `inventory/stock-doc.service.ts` 新增 `operatorStats()`；② `stock-doc.admin.resolver.ts` 暴露 `stockDocOperatorStats`（`@Allow` 与 `stockDocList` 同口径，避免租户侧被整体拦截）；③ `plugin.ts` admin SDL 增 `type StockDocOperatorStat` + `extend type Query`。**构建坑**：vendure 根 `npm run build` 会构建全部包并失败（`Failed tasks: @vendure/common:build`）→ 正确做法 `npm run build --workspace @vendure/cjk-plugin`。**前端落地（`web-admin` 4 处）**：① `src/apis/stock-doc.ts` 新增 `fetchStockDocOperatorStats`（含「为何不用 `fetchStockDocList`」的 JSDoc）；② `pages/data/dashboard/index.vue` ⑤ 段改走聚合（带 `from/to`）；③ `src/utils/ops-report.ts` 的 `groupByOperator()` / `opsCountableDocs()` **成为死代码已删除**；④ `src/utils/ops-report.spec.ts` 对应 3 条单测删除（`node --test` **11/11 PASS**）——该口径自此无纯函数单测覆盖，改由 e2e 守护。**验收（三态可跑，`_e2e/_verify_d46_ops_counter_window.py`；含可逆生产 fixture）**：向 t2 插 **120 条** `D46T-` 前缀的 `stock_doc` 头（**无明细行 → 完全不动库存/流水/成本货值**，备注标「可忽略」；`createdAt` **全部新于**真实单据 → 旧前端「最新 100 条」被合成单占满、真实作业员**必然**消失，缺陷稳定复现而非撞运气；`stock_doc` 仅 7 列、无 `customFields`，可直接 INSERT，避免 120 次 `createStockDoc` 造成大量不可删的明细/台账污染）。三态实测：**before**（旧前端）`UI 仅 D46测试员 100 单据、真实作业员 15 整行消失`；**after**（部署后）`D46测试员 120 单据 + 15 19 单据 / 29 件`（接口层 `{"D46测试员":[120,0],"15":[19,29]}` 与 UI 同源同值）；**baseline**（`DELETE FROM stock_doc WHERE "tenantChannelId"='t2' AND code LIKE 'D46T-%'` 删 120 行、t2 回 31 行、残留 0 后复跑）`totalItems 31、仅 15 19 单据 / 29 件` —— 三态均 `0 失败 / 退出码 0`。**「15 → 19 单 / 29 件」正是 D43 口径**（31 条真实单减 12 张 `STOCKTAKE`），三态一致即证明**本次只消除截断、未改变统计口径**。部署：后端本地 build → push → 服务器 `git pull --ff-only` + `pm2 restart vendure`；前端 `npm run build:h5` → `node scripts/deploy.mjs`。修复手册新增 **20.15** 节（并回改 20.12 的表行 / ③ / ⑥ 三处口径迁移说明）。**边界留档**：① 拣货单数 `fetchPickBatches({pageSize:100})` 与盘库次数 `fetchStocktakeTasks({pageSize:100})` 存在**同类硬顶**（属方案 C，本轮未扩范围）；② 若后端聚合口径再变，须同步 20.12 与 20.15 的 e2e，否则该口径将失去测试守护 |
...
**批 4 执行结论（Task 4.8 / 4.9 收口）**

- **交付物**：Task 4.1（`expiresAt` 列 + SDL）、4.2（TTL / 释放 / 留痕 + `reservations` 租户条件括号修复）、4.3（`release-expired-reservations` ScheduledTask + `customFields.reservationTtlMinutes` + `runTasksInWorkerOnly: true`）、4.4（预留单前端页：宫格入口 / 5 页签 / `mm:ss` 倒计时 / 手工释放）、4.5（批次状态机扩 3 态 + `handoverPickBatch` / `registerPickBatchException`）、4.6（批次详情 UI 状态机与只读态）、4.7（数据看板「作业分析」4 KPI + 7 日差异趋势 + 作业员明细 + CSV 导出）、4.8（验收脚本 + 手册第 20 章，前端仓 commit `d5c3ab6`）、4.9（运维章节 20.8 + 生产部署 + 线上手册 `op-40`）。
- **批次门禁全部通过**：`npm run build:h5` EXIT=0；`npm run verify:manual` PASS；e2e `_e2e/_verify_gap4_batch4.py` **PASS（失败 0 / SKIP 0）**；后端 `npx tsc -p packages/dev-server/tsconfig.prod.json` EXIT=0。
- **验收覆盖（A/B 两组）**：A1–A6 批次 `PRINTED → SHIPPED → HANDOVER → EXCEPTION → REVIEWED` 全链（含「未交接前不显示交接对象/时间」「复核后整页只读」「已完成/进行中分 Tab」），且 UI 与 API **逐字一致**；B1–B6 预留单宫格真实跳转、默认页签 `UI 3 == API totalItems 3`、`mm:ss` 每秒递减（`43:56 → 43:54`）、已释放终态无「释放」按钮、经营数据回归、**差异率 KPI == Σ|差异|/Σ应盘 手算值**（`nums=['0','0','4','3.95%']`，唯一有差异日 `2026-09-24 → 3/76`）、CSV 导出 `ops-report-7d-2026-09-26.csv`（8 行 = 表头 + 7 日）。
- **证据**：`docs/verify/gap4-batch4-*.png` 共 15 张（390×844 / dpr=2 = 780×1688，逐张人工看图核对为真实页面）；各段 console **0 error / 0 pageerror**。另有 Task 4.9 **线上手册**取证 4 张（`gap4-ops-manual-{01-toc-op40,02-chapter-top,03-selfcheck,04-faq-orphan-channel}.png`，同手机视口，见 D37/D38）。
- **缺陷与经验沉淀**：① D28 双渠道拆分（脚本缺陷）；② D29/D30 留痕的**检索口径**与**前置条件**（是预留单编号不是订单号；渠道须有可用仓）；③ D31 是本批**最有价值的部署级发现**——入库 `dist/` 与 src 漂移会让生产静默跑旧配置，规则已沉淀；④ D35 破除「schema 不匹配 = 迁移没跑」的误判（真实机制是 `synchronize: true` 的 Auto-Sync）；⑤ D36 的教训是「生产造 fixture 前先核实渠道码」，其暴露的边界（渠道删/改名后存量预留单永不释放）已由 **D38 修复并生产验证**。
- **生产部署实况**：后端仓 commit `41cdd1f8b`（`dist/` 重建 + 新增 `prod-start-worker.js`）→ 服务器 `git pull --ff-only`（14 commits / 67 files）→ `pm2 restart vendure` → **新增并 `pm2 save` `vendure-worker`**（id 4 / fork / online，注册 12 个定时任务并每分钟执行 `release-expired-reservations` → `completed successfully`）→ 生产端到端冒烟 `PENDING_ALLOC → RELEASED`（`RELEASED_OK`，测试行已清理）。前端 web-admin 走 `node scripts/deploy.mjs`（脚本内建 `build:h5`）。**D38 补修再部署**：后端 `df8f43ed4`（`cjk-plugin` 的 3 个 src 文件 + `lib` 产物重建入库）→ 服务器 `git pull --ff-only` → `pm2 restart vendure` + `pm2 restart vendure-worker` → 两进程 online（10s uptime 核对）。
- **测试数据副作用（仅本地库 + 生产已清理）**：本地 fixture 由 `_tmp_seed_resv.mjs` 直插（用完即删）；生产冒烟插入的 `orderId=999902` 预留单及其 `RES-*` 流水在验证后**已删除**，生产库无残留测试数据（D38 的 `orderId=999903` 孤儿渠道单同样验证后即删）；后端业务代码改动：D33 新增 worker 入口脚本、D38 修改 `cjk-plugin` 的预留释放链路（src 3 文件 + `lib` 产物）。
- **本批未做（属范围外）**：「渠道被删/改名后其存量预留单永不释放」的边界**已于 D38 补修并生产验证**；共享组件 `TrendChart` 的既有渲染缺陷**已于 D40 追单修复**（修复手册 20.9 + 探针级验收脚本，缺陷态 5 失败 / 修复态 0 失败）。两项原均为「本批未做」，用户追单后完成。
