...
| D41 | 线上复验中发现的**独立待办（计划外，本次未修）** | 线上复验 D25 时，`data` 态的「0 console.error」断言命中 1 条：`fetchInventoryHealth failed … You are not currently authorized to perform this action` | 该账号（`guoxinnanshan@163.com`；`myTenantAccess` 可见渠道含 `t2`）对 `inventoryHealth` 无权限，界面表现为「经营数据」看板最下方「库存健康」卡的「缺货 / 总SKU」显示为「–」。**已排除是渠道注入造成**：加 `WA_D25_NO_INJECT=1`（不注入渠道、沿用登录后默认渠道）复跑，FORBIDDEN **反而从 1 条扩到 3 条**（`dashboardOverview` / `inventoryHealth` / `categoryTop` 全部 `FORBIDDEN`）→ 属**账号角色权限**问题，与 `TrendChart` 渲染完全无关 | 待办（待用户定夺后另起任务）：① 核实「经营数据」看板三个查询各自要求的权限，以及租户管理员角色是否应默认持有；② 若该角色**不应**持有，则前端应在无权时**隐藏对应卡片**，而不是渲染「–」并在控制台打 `console.error`（当前实现让用户看到一个半残看板）。本次**不改**（超出 D25 范围） |
