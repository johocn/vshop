...
| D12 | 待回填 | `diff-a4` / `big-a4` 基线钉住的任务 `id` / `code` / 行数，以及录制时的 Chromium 版本 | 执行 Task 16 Step 3 时写入 `_e2e/baselines/print/*/env.json` 并回填到修复手册 |
| D13 | 待回填 | 草稿截图（T47/T48）在「生产只读 + 无现成草稿」下的 SKIP 记录，或本地/非生产环境补测结论 | 执行 Task 18 时如实记录；非生产环境跑了就写明环境与任务号 |
...
| D28 | 待回填（承接 D27，同因） | Task 18 第 3059 行的草稿截图脚本用**裸 HTTP body** 直接 POST，绕过了 `apis/stocktake.ts` 的归一逻辑，其 `scope` 里同样带了 `autoSplitByZone` | 执行 Task 18 时必须把该字段从 `scope` 移到顶层 `autoSplitByZone`，否则该请求会被 GraphQL 拒绝（与 D27 同一根因）；执行后回填实测结果 |
